...
|---|---|---|---|---|---|
| `img/` | COCO由来の劣化なし通常写真（晴れ） | 110 | ○ | 評価（晴れ） | コミット済み。作業不要 |
| `img_bus_rain/` | 雨天の本物写真 | 102 | ○ | 評価（雨） | コミット済み。作業不要 |
| `busbus/` | クリーンなbus原本 | 95 | ○ | フィルタ妥当性評価（FID）の適用元 | コミット済み。作業不要 |
...
- git管理するのは、再現に必要で容量が小さい評価データ `img/`・`img_bus_rain/`・`busbus/` と、デモ入力 `samples/` だけ。
- 学習データ `dataset/`、アブレーション生成物 `dataset_ablation/`、COCO取得物 `coco/`、本物タイル `real_recaptcha/`、分割結果 `test_images/` は、再生成・再取得できる大容量データなのでgit管理しない。
