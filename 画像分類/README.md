...
| **本物 (real_recaptcha)** | 6693/6693 | 0.875 | **0.896** | **+0.021** |

※ 評価のばらつき（評価セットに対する不確実性）を評価するため、ペアド・ブートストラップ法（10,000回リサンプル）により ΔAP の 95% 信頼区間を算出したところ、本物 (real_recaptcha) では [+0.015, +0.027]（0を跨がない）となり、FTによる改善が統計的に有意であることを確認した。晴れ (img) では [-0.167, 0.000]（0を跨ぐ）、雨 (img_bus_rain) では [-0.092, -0.017]（0を跨がない）であった。
